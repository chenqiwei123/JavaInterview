...
select
    request_at as Day ,
convert(sum(num)/count(1),decimal(10,2)) as "Cancellation Rate"

from
    (
    select request_at,status,case  when status='completed' then 0
    else 1 end as num  from Trips
    ...
    ) a where 1=1 group by a.request_at
